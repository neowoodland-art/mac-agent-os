import{t as e}from"./index-BqcSks_W.js";import{t}from"./account-selector-D87mja1n.js";var n=null,r={},i=[],a=[],o=``,s={sharer:`分享型`,questioner:`提问型`,sympathizer:`共情型`,skeptic:`质疑型`,sufferer:`患者型`,filler:`灌水型`,expert_ref:`推荐型`};async function c(s){o=s.id||`cw`,i=[],a=[],s.innerHTML=`
  ...
  `).join(``),document.getElementById(`cwRoleSection_${o}`).style.display=`block`)}function u(e){return{filler:`#8b8fa3`,funny:`#a78bfa`,sympathizer:`#f59e0b`,sharer:`#22c55e`,angry:`#ef4444`,oppose:`#dc2626`,guide_analyze:`#3b82f6`,guide_share:`#06b6d4`,guide_agree:`#10b981`,guide_benefit:`#f97316`,guide_compare:`#8b5cf6`}[e]||`#3b82f6`}window._cwRolePct=(e,t,n)=>{r[t]=parseInt(n)/100,document.getElementById(`cwPct_${t}_${e}`).textContent=`${n}%`;let i=Object.values(r).reduce((e,t)=>e+t,0),a=document.getElementById(`cwPctStatus_${e}`);a&&(a.textContent=`总计 ${(i*100).toFixed(0)}% ${Math.abs(i-1)<.01?`✅`:`⚠️`}`)},window._cwParseUrls=e=>{let t=document.getElementById(`cwUrls_${e}`)?.value?.trim();if(!t){alert(`请先粘贴视频链接`);return}let n=t.split(`
`).map(e=>e.trim()).filter(Boolean);if(!n.length){alert(`没有有效的链接`);return}n.forEach(e=>{i.find(t=>t.url===e)||i.push({url:e,title:``,tags:``,checked:!0,industry:``,direction:`auto`})}),d(e)},window._cwShowApiImport=e=>{let t=document.getElementById(`cwImportPanel_${e}`);t.style.display=t.style.display===`none`?`block`:`none`},window._cwApiImport=async t=>{let n=document.getElementById(`cwApiUrl_${t}`)?.value?.trim(),r=parseInt(document.getElementById(`cwApiPages_${t}`)?.value||`3`);if(!n){alert(`请输入 API 地址`);return}let a=document.getElementById(`cwApiImportStatus_${t}`);a.textContent=`⏳ 导入中...`;let o=0;for(let t=1;t<=r;t++)try{let r=await e(`/ops/import-topics`,{method:`POST`,body:JSON.stringify({api_url:n,page:t,page_size:10})});r.status===`ok`&&r.items&&r.items.length&&r.items.forEach(e=>{let t=e.url||e.link||e.title;i.find(e=>e.url===t)||(i.push({url:t,title:e.title||e.content||``,tags:Array.isArray(e.tags)?e.tags.join(`,`):e.tags||``,checked:!0,industry:``,direction:`auto`}),o++)})}catch(e){console.warn(`导入第${t}页失败:`,e)}a.textContent=o>0?`✅ 导入 ${o} 条`:`⚠️ 未获取到视频`,o>0&&d(t)};function d(e){let t=document.getElementById(`cwVideoList_${e}`);if(t){if(!i.length){t.style.display=`none`;return}t.style.display=`block`,t.innerHTML=`
    <div style="display:flex;align-items:center;gap:4px;margin-bottom:4px;font-size:10px;color:var(--text2)">
      <label style="display:flex;align-items:center;gap:2px;cursor:pointer">
        <input type="checkbox" onchange="window._cwToggleAllVideo('${e}',this.checked)" ${i.every(e=>e.checked)?`checked`:``}>
        全选
      </label>
      <span>共 ${i.length} 个视频</span>
      <button onclick="window._cwClearVideos('${e}')" style="margin-left:auto;background:none;border:none;cursor:pointer;font-size:10px;color:var(--red)">清空</button>
    </div>
    ${i.map((t,n)=>{let r=f.map(e=>`<option value="${e.id}" ${(t.industry||``)===e.id?`selected`:``}>${e.label.replace(/^[^\s]+\s/,``)}</option>`).join(``),i=p.map(e=>`<option value="${e.id}" ${(t.content_type||``)===e.id?`selected`:``}>${e.label.replace(/^[^\s]+\s/,``)}</option>`).join(``);return`
      <div style="display:flex;align-items:center;gap:3px;padding:2px 4px;font-size:10px;background:var(--bg2);border-radius:3px;margin-bottom:2px">
        <input type="checkbox" ${t.checked?`checked`:``} onchange="window._cwToggleVideo('${e}',${n},this.checked)">
        <span style="flex:2;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:60px" title="${t.title||t.url}">${t.title?t.title.slice(0,18):h(t.url,25)}</span>
        <select onchange="window._cwSetVideoIndustry('${e}',${n},this.value)" style="width:58px;background:var(--bg3);border:1px solid var(--border);color:var(--text);padding:1px 2px;border-radius:2px;font-size:9px">${r}</select>
        <select onchange="window._cwSetVideoContentType('${e}',${n},this.value)" style="width:52px;background:var(--bg3);border:1px solid var(--border);color:var(--text);padding:1px 2px;border-radius:2px;font-size:9px">${i}</select>
        <input type="text" placeholder="标签" value="${t.tags||``}" maxlength="16"
  ...
  `}}window._cwToggleVideo=(e,t,n)=>{i[t]&&(i[t].checked=n)},window._cwToggleAllVideo=(e,t)=>{i.forEach(e=>e.checked=t),d(e)},window._cwClearVideos=e=>{i=[],d(e)},window._cwSetVideoTag=(e,t,n)=>{i[t]&&(i[t].tags=n)},window._cwRemoveVideo=(e,t)=>{i.splice(t,1),d(e)},window._cwSetVideoIndustry=(e,t,n)=>{i[t]&&(i[t].industry=n)},window._cwSetVideoContentType=(e,t,n)=>{i[t]&&(i[t].content_type=n)};var f=[{id:``,label:`自动识别`},{id:`medical`,label:`🏥 医疗健康`},{id:`suzhou`,label:`📍 苏州本地`},{id:`entertainment`,label:`🎮 休闲娱乐`},{id:`news`,label:`📰 热点实讯`},{id:`food`,label:`🍜 吃喝玩乐`},{id:`patriotic`,label:`🇨🇳 家国情怀`},{id:`misc`,label:`📦 其他灌水`}],p=[{id:``,label:`自动`},{id:`real_person`,label:`🎭 真人出镜`},{id:`ad`,label:`📢 纯广告`},{id:`knowledge`,label:`📚 知识分享`},{id:`experience`,label:`💡 经验分享`},{id:`explore`,label:`🔍 探店`},{id:`review`,label:`📊 测评`},{id:`other`,label:`其他`}],m=[{id:`filler`,label:`灌水/路过`,group:`filler`,pct:.3,desc:`路过灌水`},{id:`funny`,label:`😜 搞怪`,group:`funny`,pct:.08,desc:`搞怪吐槽`},{id:`sympathizer`,label:`感同身受`,group:`emotion`,pct:.12,desc:`共情感同身受`},{id:`sharer`,label:`喜欢赞扬`,group:`emotion`,pct:.12,desc:`赞美赞扬`},{id:`angry`,label:`愤世嫉俗`,group:`emotion`,pct:.08,desc:`愤怒吐槽现实`},{id:`oppose`,label:`坚决抵制`,group:`emotion`,pct:.05,desc:`反对抵制`},{id:`guide_analyze`,label:`客观分析引导`,group:`guide`,pct:.08,desc:`客观分析夸优点引导`},{id:`guide_share`,label:`经验分享引导`,group:`guide`,pct:.06,desc:`相同经历经验分享引导`},{id:`guide_agree`,label:`赞同博主引导`,group:`guide`,pct:.04,desc:`赞同博主引导`},{id:`guide_benefit`,label:`薅羊毛推荐引导`,group:`guide`,pct:.04,desc:`利益分享薅羊毛引导`},{id:`guide_compare`,label:`对比反证引导`,group:`guide`,pct:.03,desc:`对比其他差评反证引导`}];function h(e,t){if(!e)return``;let n=e.replace(/^https?:\/\//,``).replace(/^www\./,``);return n.length>t?n.slice(0,t)+`…`:n}window._cwEditVideo=(e,t)=>{let n=i[t];if(!n)return;let r=document.createElement(`div`);r.style.cssText=`position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.4);z-index:99999;display:flex;align-items:center;justify-content:center`,r.onclick=e=>{e.target===r&&r.remove()};let a=f.map(e=>`<option value="${e.id}" ${n.industry===e.id?`selected`:``}>${e.label}</option>`).join(``),o=DIRECTIONS.map(e=>`<option value="${e.id}" ${(n.direction||`auto`)===e.id?`selected`:``}>${e.label}</option>`).join(``);r.innerHTML=`
    ...
    </div>`,document.body.appendChild(r)},window._cwSaveVideoEdit=(e,t)=>{let n=i[t];n&&(n.industry=document.getElementById(`cwEditInd_${e}`)?.value||``,n.tags=document.getElementById(`cwEditTags_${e}`)?.value?.trim()||``,n.direction=document.getElementById(`cwEditDir_${e}`)?.value||`auto`,document.querySelector(`[style*="position:fixed"]`)?.remove(),d(e))},window._cwGenerate=async e=>{let t=i.filter(e=>e.checked);if(!t.length){alert(`请至少选择一个视频`);return}let o=n?n.getSelected():[];if(!o||!o.length){alert(`请至少选择一个账号`);return}let c=parseInt(document.getElementById(`cwPerVideo_${e}`)?.value||`15`),l=parseInt(document.getElementById(`cwLongRatio_${e}`)?.value||`20`)/100,u=document.getElementById(`cwAiEnhance_${e}`)?.checked||!1,d=c*t.length;if(c>o.length){let e=c-o.length;if(!confirm(`每个视频生成 ${c} 条评论，但只有 ${o.length} 个账号。每视频多 ${e} 条将不分配账号。继续？`))return}let f=document.getElementById(`cwGenerateBtn_${e}`);f.textContent=`⏳ 生成中...`,f.disabled=!0,a=[];let p=t.map(e=>e.title||``).filter(Boolean).join(` | `),m=[...new Set(t.flatMap(e=>(e.tags||``).split(/[,，]/).map(e=>e.trim()).filter(Boolean)))],h=[...new Set(t.map(e=>e.industry||``).filter(Boolean))],v=h.length===1?h[0]:``,y=[...new Set(t.map(e=>e.direction||`auto`).filter(e=>e!==`auto`))],b=y.length===1?y[0]:`auto`;try{let e=(await(await fetch(`/api/comment-workbench/generate`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({video_title:p||`视频`,video_tags:m,video_industry:v,direction:b,role_distribution:r,total:d,ai_enhance:u,long_ratio:l})})).json()).comments||[];for(let n=0;n<t.length;n++){let r=t[n],i=n*c,l=Math.min(i+c,e.length),u=e.slice(i,l),d=g(r.title||``),f=u;d&&(f=u.map(e=>({...e,text:e.text?e.text.replace(/\{keyword\}/g,d):e.text}))),f.forEach((e,t)=>{let r=t%o.length;a.push({video_idx:n,account_idx:r,account_id:o[r]?.id||o[r]||`account_${r}`,text:e.text,role:e.role,role_label:e.role_label||s[e.role]||e.role,is_long:!!e.is_long})})}}catch(e){console.error(`生成评论失败:`,e),alert(`❌ 生成失败: `+(e.message||e))}f.textContent=`🚀 生成评论`,f.disabled=!1,_(e)};function g(e){if(!e)return``;let t=e.match(/(?:关于|分享|推荐|说说|聊聊|如何|怎么|什么|为什么)(.{2,8})/);return t?t[1]:e.replace(/[的在了是就都和与及、，。！？]/g,` `).split(/\s+/).filter(Boolean)[0]||e.slice(0,6)}function _(e){let t=document.getElementById(`cwPreview_${e}`),r=document.getElementById(`cwCommentList_${e}`),o=document.getElementById(`cwPreviewCount_${e}`);if(!t||!r)return;if(!a.length){t.style.display=`none`;return}t.style.display=`block`,o.textContent=`共 ${a.length} 条 | ${i.filter(e=>e.checked).length} 个视频 | ${n?.getSelected()?.length||0} 个账号`;let s=i.filter(e=>e.checked);r.innerHTML=a.map((t,n)=>{let r=s[t.video_idx],i=r?r.title?r.title.slice(0,20):h(r.url,20):`视频#${t.video_idx+1}`,a=u(t.role);return`
      <div style="display:flex;align-items:center;gap:4px;padding:4px 8px;background:var(--bg3);border-radius:4px;font-size:10px;border-left:3px solid ${a}">
        <span style="font-size:9px;color:var(--text2);min-width:16px">#${n+1}</span>
        <span style="font-size:9px;color:var(--text2);min-width:40px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${r?r.title||r.url:``}">📹 ${i}</span>
        <span style="font-size:9px;padding:1px 4px;border-radius:3px;background:${a}20;color:${a};min-width:35px;text-align:center">${t.role_label}${t.is_long?`📖`:``}</span>
        <span style="font-size:9px;color:var(--text2);min-width:60px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">👤 ${t.account_id}</span>
        <span style="flex:1;color:var(--text);min-width:0">${v(t.text)}</span>
        <button onclick="window._cwEditComment(${n},'${e}')" style="background:none;border:none;cursor:pointer;font-size:11px;opacity:.5" title="编辑">✏️</button>
        <button onclick="window._cwDeleteComment(${n},'${e}')" style="background:none;border:none;cursor:pointer;font-size:11px;opacity:.3" title="删除">✕</button>
    ...
    </div>`,document.body.appendChild(r)},window._cwSaveEdit=(e,t)=>{let n=document.getElementById(`cwEditText_${t}`)?.value?.trim();n&&(a[e]&&(a[e].text=n),document.querySelector(`[style*="position:fixed"]`)?.remove(),_(t))},window._cwDeleteComment=(e,t)=>{a.splice(e,1),_(t)},window._cwRegenerate=e=>{a=[],window._cwGenerate(e)},window._cwDispatch=async e=>{if(!a.length){alert(`没有可分发的内容`);return}if(!confirm(`确认分发 ${a.length} 条评论？`))return;let t=document.getElementById(`cwResult_${e}`),r=document.getElementById(`cwResultContent_${e}`);t.style.display=`block`,r.innerHTML=`⏳ 分发中...`;let o=i.filter(e=>e.checked);n&&n.getSelected();let s=0,c=0,l=[];for(let e=0;e<a.length;e++){let t=a[e],n=o[t.video_idx];r.innerHTML=`⏳ 分发中 ${e+1}/${a.length}...`;try{let r=await(await fetch(`/api/ops/run`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({type:`smart_comment`,accounts:[t.account_id],params:{urls:[n?n.url:``],direction:`praise`,custom_comment:t.text,skip_analysis:!0}})})).json();r.status===`accepted`||r.status===`ok`?(s++,l.push(`#${e+1} ${t.account_id} [${t.role_label}] ✅`)):(c++,l.push(`#${e+1} ${t.account_id} ❌ ${r.message||r.error||``}`))}catch(n){c++,l.push(`#${e+1} ${t.account_id} ❌ ${n.message}`)}}r.innerHTML=`
