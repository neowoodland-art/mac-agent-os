async function e(e){e.innerHTML=`<div id="schedPanel" style="padding:20px"><div class="loading">⏳ 加载定时任务...</div></div>`;try{let t=(await(await fetch(`/api/matrix/schedules`)).json()).schedules||[];e.innerHTML=`
      <div style="padding:20px">
        ...
        <div style="background:var(--bg2);border-radius:var(--radius);padding:12px;border:1px solid var(--border);margin-bottom:12px">
          ...
          <div style="font-size:12px">${Array.isArray(t)&&t.length?t.map(e=>`<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border)">
