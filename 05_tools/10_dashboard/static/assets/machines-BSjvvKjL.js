import{i as e,r as t,t as n}from"./router-BSIHrp4B.js";async function r(r){e(r,`读取联邦心跳中...`);try{let e=(await n(`/machines`)).machines||[],t=document.getElementById(`machineCount`);if(t&&(t.textContent=`共 ${e.length} 台主机`),!e.length){r.innerHTML=`<div class="error" style="padding:40px">无心跳数据</div>`;return}r.innerHTML=`
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:16px" id="machineGrid">
        ${e.map(e=>i(e)).join(``)}
      </div>
    `}catch(e){t(r,`加载失败: ${e.message}`)}}function i(e){let t=e.status===`online`||e._live===!0,n=e.status===`recent`,r=e._live===!0,i=r?`#1D9E75`:t?`#22C55E`:n?`#F59E0B`:`#EF4444`,a=e._last_push_sec||0,o=e.minutes_ago||Math.round(a/60)||999,s=r?`${a}秒前`:o<60?`${o} 分钟前`:`${(o/60).toFixed(1)} 小时前`,c=e.disk_used_gb||0,l=e.disk_total_gb||1,u=Math.round(c/l*100),d=`
