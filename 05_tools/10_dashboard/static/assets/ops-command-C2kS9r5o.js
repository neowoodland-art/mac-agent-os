import{t as e}from"./index-BFzzaMkr.js";var t=null,n=[];async function r(t){try{n=(await e(`/v2/accounts`)).accounts||[]}catch{n=[]}t.innerHTML=`
    ...
    </div>`,window._cmdRegistered2||(window._cmdRegistered2=!0,window._cmdRefresh=()=>d(t),window._cmdStop=async(n,r)=>{if(confirm(`停止 ${r} ?`))try{await e(`/ops/task/cancel`,{method:`POST`,body:JSON.stringify({task_id:r,machine:n})}),alert(`✅ 已停止`),d(t)}catch(e){alert(`❌ `+e.message)}},window._cmdRemoveQueue=(e,t)=>alert(`移出队列功能开发中`),window._cmdToggleAuto=()=>{document.getElementById(`cmdAutoRefresh`)?.checked?a(t):o()},window._cmdResetMachine=async()=>{let n=document.getElementById(`cmdResetMachine`)?.value||``,r=n?`确定初始化机器 ${n}？\n将杀掉所有活跃任务和浏览器进程`:`请先选择要初始化的机器`;if(!n){alert(r);return}if(!confirm(r))return;let i=document.getElementById(`cmdResetResult`);i&&(i.textContent=`⏳ 执行中...`);try{await e(`/ops/reset`,{method:`POST`,body:JSON.stringify({machine:n})}),i&&(i.textContent=`✅ ${n} 已初始化`),d(t)}catch(e){i&&(i.textContent=`❌ `+e.message)}},window._cmdResetAll=async()=>{if(!confirm(`⚠️ 确定重置全部机器？
将杀掉所有机器的活跃任务和浏览器进程`))return;let n=document.getElementById(`cmdResetResult`);n&&(n.textContent=`⏳ 重置全部...`);try{let r=await e(`/ops/reset`,{method:`POST`,body:JSON.stringify({})});n&&(n.textContent=`✅ 已重置 ${Object.keys(r.machines||{}).length} 台机器`),d(t)}catch(e){n&&(n.textContent=`❌ `+e.message)}},window._cmdRefresh()),await d(t),a(t)}function i(){o()}function a(e){o(),t=setInterval(()=>{document.getElementById(`cmdAutoRefresh`)?.checked&&d(e,!0)},15e3)}function o(){t&&=(clearInterval(t),null)}function s(e){return n.find(t=>t.id===e)||{}}function c(e){if(!e||e<=0)return``;let t=Date.now()/1e3;if(e>t)return`刚刚`;let n=Math.floor(t-e);return n<5?`刚刚`:n<60?`${n}秒`:n<3600?`${Math.floor(n/60)}分${n%60}秒`:`${Math.floor(n/3600)}时${Math.floor(n%3600/60)}分`}function l(e,t){return!t||t<=0?``:`${Math.round(e/t*100)}%`}function u(e){if(!e.account_id)return`#8b8fa3`;let t=e.health||`healthy`;return t===`healthy`?`#22c55e`:t===`warning`?`#eab308`:`#ef4444`}async function d(t,n=!1){try{let[t,r]=await Promise.all([e(`/ops/queue`),e(`/ops/machines`)]);if(!n){let e=document.getElementById(`cmdLastUpdate`);e&&(e.textContent=`🕐 ${new Date().toLocaleTimeString()}`)}let i=t?.machines||{},a=Object.keys(i);for(let e of[`cmdMachineFilter`,`cmdResetMachine`]){let t=document.getElementById(e);t&&t.options.length<=1&&a.forEach(e=>{let n=document.createElement(`option`);n.value=e,n.textContent=e,t.appendChild(n)})}f(t),p(t),m(t)}catch(e){n||(document.getElementById(`cmdTrackGrid`).innerHTML=`<div style="color:#ef4444;font-size:12px;grid-column:1/-1;padding:20px;text-align:center">❌ 加载失败: ${e.message}</div>`)}}async function f(t){let n=document.getElementById(`cmdAlerts`),r=document.getElementById(`cmdAlertsList`);if(!n||!r)return;let i=[],a=t?.machines||{};for(let[e,t]of Object.entries(a)){let n=t?.counts||{};t?.error&&i.push({machine:e,level:`error`,msg:`🔴 不可达: ${t.error}`}),n?.failed>0&&i.push({machine:e,level:`warning`,msg:`❌ ${n.failed} 个任务失败`})}try{let t=await e(`/v2/status-summary`);t.banned>0&&i.push({machine:`全域`,level:`error`,msg:`⛔ ${t.banned} 个账号已封号`}),t.cookie_expiring>0&&i.push({machine:`全域`,level:`warning`,msg:`🟡 ${t.cookie_expiring} 个账号 Cookie 即将过期`}),t.no_cookie>0&&i.push({machine:`全域`,level:`info`,msg:`🔴 ${t.no_cookie} 个账号无 Cookie`}),t.unknown>0&&i.push({machine:`全域`,level:`info`,msg:`⚪ ${t.unknown} 个账号状态未知`})}catch{}i.length>0?(n.style.display=`block`,document.querySelector(`.cmdFilter[value="P0"]`)?.checked,document.querySelector(`.cmdFilter[value="P1"]`)?.checked,document.querySelector(`.cmdFilter[value="P2"]`)?.checked,r.innerHTML=i.map(e=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:4px 8px;margin-bottom:2px;background:${e.level===`error`?`#fef2f2`:e.level===`warning`?`#fffbeb`:`#f8fafc`};border:1px solid ${e.level===`error`?`#fecaca`:e.level===`warning`?`#fde68a`:`#e2e8f0`};border-radius:4px;font-size:10px">
