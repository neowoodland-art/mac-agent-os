import{n as e,t}from"./index-CmNB3DzK.js";import{t as n}from"./account-selector-B-X_KS9a.js";var r=null;async function i(e){let i=e.id||`comment`;e.innerHTML=`
    ...
    </div>`;try{let e=await t(`/matrix/accounts`),a=Array.isArray(e)?e:e.accounts||[],o=document.getElementById(`acctList_${i}`);o&&(r=n(o,{accounts:a,checkAll:!1,height:`200px`,showBlueprint:!1,showCollectTime:!1}))}catch(e){let t=document.getElementById(`acctList_${i}`);t&&(t.innerHTML=`<div class="error">❌ ${e.message}</div>`)}a(i)}function a(n){window._commentGlobals||(window._commentGlobals=!0,window._runComment=async function(){let i=r?.getSelected()||[],a=document.getElementById(`urls_${n}`)?.value,o=document.getElementById(`dir_${n}`)?.value,s=document.getElementById(`corpus_${n}`)?.value,c=document.getElementById(`result_${n}`);if(!a||!a.trim()){c.textContent=`请填写至少一个视频链接`;return}let l=a.split(`
