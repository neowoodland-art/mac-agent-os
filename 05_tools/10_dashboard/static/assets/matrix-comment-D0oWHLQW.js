import{n as e,t}from"./index-BOUS2oCC.js";import{t as n}from"./account-selector-D87mja1n.js";var r=null,i=``,a=[];async function o(e,o){i=e.id||`sc`,e.innerHTML=`
...
...`:``}`))return;let c=document.getElementById(`result_${n}`);c.textContent=`⏳ 正在分发 ${s} 条任务...`;let l=document.getElementById(`interval_${n}`)?.value||`60-180`;try{await t(`/ops/run`,{method:`POST`,body:JSON.stringify({type:`smart_comment`,accounts:i.map(e=>e.id),params:{urls:Object.keys(o),comments:o,direction:document.getElementById(`dir_${n}`)?.value||`praise`,interval:l}})}),c.textContent=`✅ 已分发 ${s} 条评论任务 (P0)`}catch(e){c.textContent=`❌ 分发失败: ${e.message}`}},window._sc_go=async n=>{let i=r?.getSelected()||[],a=document.getElementById(`urls_${n}`)?.value,o=document.getElementById(`dir_${n}`)?.value,s=document.getElementById(`corpus_${n}`)?.value||``,c=document.getElementById(`result_${n}`);if(!a||!a.trim()){c.textContent=`请填写至少一个视频链接`;return}let l=a.split(`
`).map(e=>e.trim()).filter(e=>e);if(!i.length){c.textContent=`请先选择账号`;return}let u=i.length*l.length,d=`账号 ${i.length} 个 · 视频 ${l.length} 个\n方向: ${o} · 语料: ${s||`自动`}`;if(!await e(`即将一键分析并分发 ${u} 条评论`,d)){c.textContent=`已取消`;return}let f=document.getElementById(`interval_${n}`)?.value||`60-180`;c.textContent=`⏳ 分析 ${l.length} 个视频并分发到 ${i.length} 个账号...`;try{let e=await t(`/ops/run`,{method:`POST`,body:JSON.stringify({type:`smart_comment`,accounts:i.map(e=>e.id),params:{urls:l,direction:o,corpus_category:s||void 0,interval:f,preview:!1}})}),n=e?.per_machine||{},r=``,a=Object.keys(n).length>0;for(let[e,t]of Object.entries(n)){let n=t?.success||0,i=t?.failed||0;r+=`🖥 ${e}: ✅${n} ❌${i}\n`}let d=e?.commands?.length||0;c.textContent=a&&r||(d>0?`✅ 已提交 ${d} 条评论任务 (P0)，正在排队执行...`:`✅ 已分发 ${u} 条评论任务 (P0)`)}catch(e){c.textContent=`❌ 执行失败: ${e.message}`}})}function c(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`):``}export{o as loadView};