async function e(e){let t=Math.random().toString(36).slice(2,6),n=null,r={atomOps:[],deleted:[],stepLabels:{}},i=null,a=[],o=!1;function s(){try{let e=localStorage.getItem(`rec_annotations_v4`)||`{}`;r=JSON.parse(e)[n?.name]||{atomOps:[],deleted:[],stepLabels:{}}}catch{r={atomOps:[],deleted:[],stepLabels:{}}}}function c(){if(n)try{let e=localStorage.getItem(`rec_annotations_v4`)||`{}`,t=JSON.parse(e);t[n.name]=r,localStorage.setItem(`rec_annotations_v4`,JSON.stringify(t))}catch{}}let l=0;function u(){return`ao_`+ ++l+`_`+Date.now().toString(36)}function d(e){return r.atomOps.find(t=>e>=t.stepRange[0]&&e<=t.stepRange[1])}function f(e){return r.deleted.includes(e)}function p(e){let[t,r]=e.stepRange,i=[];for(let e=t;e<=r;e++){let t=n.steps[e];if(t&&n.actMap){let e=n.actMap[t.step]||[];i.push(...e)}}let a=n.steps[t],o=n.steps[r];return{firstAct:i[0]||{},acts:i,screenshotBefore:a?.screenshot_url||``,screenshotAfter:o?n.steps[Math.min(r+1,n.steps.length-1)]?.screenshot_url:``,from:a?.page||{},to:o?.page||{},events:n.steps.slice(t,r+1).flatMap(e=>e.events||[])}}e.innerHTML=`
    <div style="padding:12px">
    ...
    </div>`,window._reloadAll=function(e){m(e),v(e),_(e)},window._recStart=function(e){h(e)},window._recStop=function(e){g(e)},window._analyzeRec=function(e,t){y(e,t)},window._deleteRec=async function(e,t){if(confirm(`确定删除录制包「`+e+`」？`))try{let n=await(await fetch(`/api/matrix/record/delete/`+encodeURIComponent(e),{method:`POST`})).json();n.status===`ok`?(document.getElementById(`recStatusMsg_`+t).textContent=`✅ 已删除: `+e,v(t)):alert(`删除失败: `+(n.message||``))}catch(e){alert(`删除失败: `+e.message)}},window._toggleSelectionMode=function(e){b(e)},window._mergeSelected=function(e){E(e)},window._deleteSelected=function(e){w(e)},window._clearSelection=function(e){x(e)},window._collapseAll=function(e){i=null,A(e)},window._toggleAo=function(e,t){i=i===e?null:e,A(t)},window._saveAnnotations=function(e){c(),document.getElementById(`recStatusMsg_`+e).textContent=`✅ 已保存`,setTimeout(()=>{let t=document.getElementById(`recStatusMsg_`+e);t&&(t.textContent=``)},2e3)},window._editAo=function(e,t){D(e,t)},window._deleteAo=function(e,t){O(e,t)},window._stepClick=function(e,t){C(e,t)},window._deleteStep=function(e,t){T(e,t)},m(t),v(t),_(t);async function m(e){let t=document.getElementById(`recAcct_${e}`);if(t)try{let e=await(await fetch(`/api/matrix/accounts`)).json();t.innerHTML=`<option value="">— 选择账号 —</option>`+(Array.isArray(e)?e:e.accounts||[]).filter(e=>e.is_local&&e.platform).map(e=>`<option value="${e.id}">${e.platform===`douyin`?`🎵`:`📕`} ${e.id}</option>`).join(``)}catch{t.innerHTML=`<option value="">加载失败</option>`}}async function h(e){let t=document.getElementById(`recAcct_${e}`)?.value;if(!t){alert(`请先选择账号`);return}let n=document.getElementById(`recStartBtn_${e}`);n.textContent=`⏳ 启动中...`,n.disabled=!0;try{let r=await(await fetch(`/api/matrix/record/start`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({account:t})})).json();r.status===`ok`?(document.getElementById(`recStatus_${e}`).textContent=`🟢 录制中 (PID ${r.pid})`,document.getElementById(`recStopBtn_${e}`).style.display=``,n.style.display=`none`):alert(r.message)}catch(e){alert(`启动失败: `+e.message)}finally{n.textContent=`▶ 开始录制`,n.disabled=!1}}async function g(e){try{await fetch(`/api/matrix/record/stop`,{method:`POST`}),document.getElementById(`recStatus_${e}`).textContent=`⏹ 已停止`,document.getElementById(`recStopBtn_${e}`).style.display=`none`,document.getElementById(`recStartBtn_${e}`).style.display=``,v(e)}catch(e){alert(`停止失败: `+e.message)}}async function _(e){try{let t=await(await fetch(`/api/matrix/record/status`)).json();t.running&&(document.getElementById(`recStatus_${e}`).textContent=`🟢 录制中 (PID ${t.pid})`,document.getElementById(`recStopBtn_${e}`).style.display=``,document.getElementById(`recStartBtn_${e}`).style.display=`none`)}catch{}}async function v(e){let t=document.getElementById(`recList_${e}`);if(t)try{let n=(await(await fetch(`/api/matrix/record/list`)).json()).recordings||[];if(!n.length){t.innerHTML=`<div style="color:var(--text2);padding:8px;text-align:center">暂无录制包</div>`;return}t.innerHTML=n.map(t=>`<div style="display:flex;align-items:center;gap:6px;padding:4px 6px;border-bottom:1px solid var(--border);font-size:10px">
      ...
      </div>`).join(``)}catch(e){t.innerHTML=`<div style="color:var(--red);padding:8px">❌ ${e.message}</div>`}}async function y(e,t){document.getElementById(`recAnalysis_${t}`).style.display=``;try{let r=await(await fetch(`/api/matrix/record/detail/`+encodeURIComponent(e))).json();n={name:e,meta:r.meta||{},steps:r.steps||[],actions:(r.analysis||{}).actions||[],suggestions:(r.analysis||{}).blueprint_suggestions||[]};let i={};n.actions.forEach(e=>{let t=e.step_after||e.step_before;i[t]||(i[t]=[]),i[t].push(e)}),n.actMap=i;let a={};n.suggestions.forEach(e=>{a[e.step]=e}),n.sugMap=a,s(),document.getElementById(`recName_${t}`).textContent=`📹 ${e}`;let o=n.meta;document.getElementById(`recMeta_${t}`).textContent=`${o.platform||`?`} · ${o.account_id||`?`} · ${n.steps.length}步 · ${Math.round(o.duration||0)}s`,A(t)}catch(e){document.getElementById(`recContent_`+t).innerHTML=`<div style="color:var(--red);padding:8px">❌ ${e.message}</div>`}}function b(e){o=!o,a=[],document.getElementById(`selModeBtn_`+e).textContent=o?`✓ 退出选择`:`☐ 选择模式`,document.getElementById(`selModeBtn_`+e).style.background=o?`rgba(99,102,241,.2)`:`var(--bg3)`,document.getElementById(`selModeBtn_`+e).style.color=o?`var(--primary)`:`var(--text)`,document.getElementById(`selToolbar_`+e).style.display=o?``:`none`,A(e)}function x(e){a=[],S(e),A(e)}function S(e){let t=a.length;document.getElementById(`selCount_`+e).textContent=t,document.getElementById(`mergeBtn_`+e).disabled=t<2,document.getElementById(`delBtn_`+e).disabled=t===0}function C(e,t){if(!o)return;let n=a.indexOf(e);n>=0?a.splice(n,1):a.push(e),S(t),A(t)}function w(e){a.length&&confirm(`确定删除这 `+a.length+` 步？`)&&(a.forEach(e=>{f(e)||r.deleted.push(e)}),a=[],c(),S(e),A(e))}function T(e,t){confirm(`确定删除步骤 `+(e+1)+`？`)&&(f(e)||r.deleted.push(e),c(),A(t))}function E(e){if(a.length<2)return;let t=[...a].sort((e,t)=>e-t);if(!t.every((e,t,n)=>t===0||e===n[t-1]+1)){alert(`只能选择连续的步骤合并为原子操作`);return}for(let e of r.atomOps){let[n,r]=e.stepRange;if(t[0]<=r&&t[t.length-1]>=n){alert(`选择的步骤与已有原子操作 "`+(e.label||e.name)+`" 重叠`);return}}let i=(n.actMap[n.steps[t[0]]?.step]?.[0]||{}).action_type||`custom_op`;k({title:`打包为原子操作`,fields:[{id:`aoName`,label:`标识名`,value:i,placeholder:`英文标识, 如 like_video`},{id:`aoLabel`,label:`中文名`,value:``,placeholder:`如 "点赞视频"`},{id:`aoNote`,label:`备注`,value:``,placeholder:`描述该原子操作的功能和注意点`,type:`textarea`},{id:`aoTags`,label:`特征标签`,value:`验证码:否, 框架:player`,placeholder:`逗号分隔，如 验证码:是, 框架:player`}],onConfirm:n=>{let o={id:u(),name:n.aoName||i,label:n.aoLabel||n.aoName||i,note:n.aoNote||``,tags:(n.aoTags||``).split(`,`).map(e=>e.trim()).filter(Boolean),stepRange:[t[0],t[t.length-1]]};r.atomOps.push(o),a=[],c(),S(e),A(e)}},e)}function D(e,t){let n=r.atomOps.find(t=>t.id===e);n&&k({title:`编辑原子操作`,fields:[{id:`aoName`,label:`标识名`,value:n.name},{id:`aoLabel`,label:`中文名`,value:n.label},{id:`aoNote`,label:`备注`,value:n.note,type:`textarea`},{id:`aoTags`,label:`特征标签`,value:(n.tags||[]).join(`, `)}],onConfirm:e=>{n.name=e.aoName||n.name,n.label=e.aoLabel||e.aoName||n.name,n.note=e.aoNote||``,n.tags=(e.aoTags||``).split(`,`).map(e=>e.trim()).filter(Boolean),c(),A(t)}},t)}function O(e,t){confirm(`确定删除此原子操作？步骤将恢复为未打包状态`)&&(r.atomOps=r.atomOps.filter(t=>t.id!==e),i===e&&(i=null),c(),A(t))}function k(e,t){let n=document.getElementById(`recAnalysisInner_`+t),r=document.createElement(`div`);r.style.cssText=`position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.4);z-index:999;display:flex;align-items:center;justify-content:center`,r.onclick=e=>{e.target===r&&r.remove()},r.innerHTML=`<div style="background:var(--bg2);border-radius:8px;padding:16px;width:380px;max-width:90vw;border:1px solid var(--border)">
      ...
      <div style="display:grid;grid-template-columns:1fr auto 1fr;gap:6px;margin-bottom:6px">
        <div style="background:rgba(37,99,235,.04);border-radius:5px;padding:5px;border:1px solid rgba(37,99,235,.1)">
          ...
          <tr><td style="color:var(--text2)">框架</td><td>${L(l.mode)}</td></tr>
        ...
        <div style="background:rgba(5,150,105,.04);border-radius:5px;padding:5px;border:1px solid rgba(5,150,105,.1)">
          ...
          <tr><td style="color:var(--text2)">框架</td><td>${L(u.mode)}</td></tr>
      ...
      ${(()=>{let e=``;for(let t=i;t<=a;t++){let i=n.steps[t];i&&(e+=F(i,t,r))}return e})()}
    ...
    `}function P(e,t,r){let i=(n.actMap[e.step]||[])[0]||{},s=n.sugMap[e.step]||{},c=e.events||[],l=o&&a.includes(t),u=i.action_type||i.action_desc||s.suggested_op||`浏览/等待`,d=``;c.some(e=>e.t===`click`)&&(d+=`🖱`),c.some(e=>e.t===`key`)&&(d+=`⌨️`),c.some(e=>e.t===`scroll`)&&(d+=`📜`),d||=`⏸`;let f=e.page||{};return`<div style="display:flex;align-items:center;gap:4px;padding:4px 6px;border-bottom:1px solid var(--border);font-size:10px;
      ...
      <span>${d}</span>
      <span style="color:var(--text2);font-size:9px;min-width:60px">${L(f.page_mode||`?`)}</span>
    ...
    </div>`}function F(e,t,r){let i=(n.actMap[e.step]||[])[0]||{},a=e.events||[],o=i.action_type||i.action_desc||`浏览/等待`,s=a.slice(0,3).map(e=>e.t===`click`?`🖱(${e.x},${e.y})`:e.t===`key`?`⌨️${e.k}`:e.t===`scroll`?`📜${Math.abs(e.dy||0)}`:e.t).join(` `),c=e.page||{};return`<div style="display:flex;align-items:center;gap:4px;padding:3px 6px;border-bottom:1px solid var(--border);font-size:9px">
      <span style="color:var(--text2);min-width:24px;font-size:8px">#${t+1}</span>
      <span style="color:var(--text2);font-size:8px;min-width:50px">${L(c.page_mode||`?`)}</span>
