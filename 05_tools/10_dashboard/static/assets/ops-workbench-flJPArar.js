async function e(e){let t=Math.random().toString(36).slice(2,6);e.innerHTML=`
    ...
    <div style="display:flex;gap:4px">
      <button id="testBtn_${t}" onclick="window._testOp('${e}','${t}')" style="flex:1;background:#22c55e;color:#fff;border:none;padding:4px;border-radius:4px;cursor:pointer;font-size:10px">🧪 测试此操作</button>
      <div id="opTestResult_${t}" style="margin-top:4px;font-size:9px;color:var(--text2)"></div>
    </div>`},window._testOp=async function(e,t){let n=document.getElementById(`testBtn_`+t),r=document.getElementById(`opTestResult_`+t);if(!(!n||!r)){n.disabled=!0,n.textContent=`⏳ 执行中...`,n.style.opacity=`0.6`,r.textContent=`请求中...`;try{let t=await(await fetch(`/api/matrix/accounts`)).json(),n=Array.isArray(t)?t:t.accounts||[],i=n.find(e=>e.is_local)||n[0];if(!i){r.textContent=`❌ 无可用账号`;return}let a=await(await fetch(`/api/ops/run`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({type:`nurture`,accounts:[i.id],params:{blueprint:`douyin_daily`,rounds:1,single_op:e},wait:!0})})).json();r.textContent=`✅ 已提交: `+(a.status||a.message||JSON.stringify(a))}catch(e){r.textContent=`❌ `+e.message}finally{n.disabled=!1,n.textContent=`🧪 测试此操作`,n.style.opacity=`1`}}};var u={},d={};try{d=JSON.parse(localStorage.getItem(`rec_names`)||`{}`)}catch{}function f(){try{localStorage.setItem(`rec_names`,JSON.stringify(d))}catch{}}async function p(e){let t=document.getElementById(`recSelect_${e}`);if(t)try{t.innerHTML=`<option value="">— 选择录制包 —</option>`+((await(await fetch(`/api/matrix/record/list`)).json()).recordings||[]).map(e=>`<option value="${e.name}">🎬 ${e.account} ${e.platform} (${e.steps}步)</option>`).join(``)}catch{t.innerHTML=`<option value="">加载失败</option>`}}async function m(e,t){let n=document.getElementById(`recSteps_${t}`);if(n)try{let r=await(await fetch(`/api/matrix/record/detail/`+encodeURIComponent(e))).json(),i=r.steps||[],a=(r.analysis||{}).actions||[],o={};a.forEach(e=>{let t=e.step_after||e.step_before;o[t]||(o[t]=[]),o[t].push(e)}),u={steps:i,actMap:o,uid:t,name:e};let s=`<div style="font-weight:600;margin-bottom:6px;font-size:11px;display:flex;justify-content:space-between">
      <span>${e}</span>
      ...
      <button onclick="window._recExport('${t}')" style="background:var(--bg2);border:1px solid var(--border);color:var(--text);padding:4px 8px;border-radius:4px;cursor:pointer;font-size:10px">📦 导出蓝图</button>
      <span id="recSaveStatus_${t}" style="font-size:9px;color:var(--text2);flex-shrink:0">${c>0?`已保存 `+c+` 步`:``}</span>
    </div>`,n.innerHTML=s,window._recShowDetail=function(e){h(e,t)},window._recSetName=function(e,t){let n=document.getElementById(`recNameInput_${e}_${t}`)?.value?.trim();n&&(d[u.name]||(d[u.name]={}),d[u.name][e]=n,document.getElementById(`recNameLabel_${e}_${t}`).textContent=n)},window._recSaveAll=function(e){u.steps.forEach((t,n)=>{let r=document.getElementById(`recNameInput_${n}_${e}`);r&&r.value.trim()&&(d[u.name]||(d[u.name]={}),d[u.name][n]=r.value.trim())}),f();let t=d[u.name]?Object.keys(d[u.name]).length:0;document.getElementById(`recSaveStatus_${e}`).textContent=`✅ 已保存 `+t+`/`+u.steps.length+` 步命名`},window._recExport=function(e){let t=d[u.name]||{},n=u.steps.map((e,n)=>({step_id:n+1,op:t[n]||``,args:{}})).filter(e=>e.op);u.name.split(`_`).slice(1,2).join(`_`);let r={name:`recording_`+u.name.split(`recording_`).pop()||`exported`,description:`从录制导出：`+u.name,platform:u.steps[0]?.page?.url?.includes(`douyin`)?`douyin`:`xiaohongshu`,steps:n},i=new Blob([JSON.stringify(r,null,2)],{type:`application/json`}),a=document.createElement(`a`);a.href=URL.createObjectURL(i),a.download=r.name+`.json`,a.click(),URL.revokeObjectURL(a.href),document.getElementById(`recSaveStatus_${e}`).textContent=`📦 蓝图已导出: `+n.length+` 步`},h(0,t)}catch(e){n.innerHTML=`<div style="color:var(--red);padding:8px">❌ ${e.message}</div>`}}function h(e,t){let n=document.getElementById(`recStepDetail_${t}`);if(!n||!u.steps)return;if(window._recActiveStep!==void 0&&window._recActiveStep!==e){let e=document.getElementById(`recNameInput_${window._recActiveStep}_${t}`);e&&e.value.trim()&&(d[u.name]||(d[u.name]={}),d[u.name][window._recActiveStep]=e.value.trim())}window._recActiveStep=e;let r=u.steps[e];if(!r)return;document.querySelectorAll(`#recSteps_${t} [data-sidx]`).forEach(t=>{let n=parseInt(t.dataset.sidx)===e;t.style.background=n?`var(--primary)`:`var(--bg3)`,t.style.color=n?`#fff`:``,t.querySelectorAll(`span`).forEach(e=>{n&&(e.style.color=e.dataset.origColor||(e.style.color===`rgb(255,255,255)`?``:`rgba(255,255,255,.7)`))})});let i=(u.actMap[r.step]||[])[0]||{},a=i.action_desc||`浏览`,o=i.suggested_op||i.action_type||`?`,s=d[u.name]?.[e]||o,c=r.screenshot_url||``,l=r.page?r.page.url:``;r.page&&(r.page.text_snippet||``).slice(0,100);let f=u.steps[e+1],p=i.features||[];n.innerHTML=`
    <div style="background:var(--bg3);border-radius:6px;padding:8px;border:1px solid var(--primary)">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
