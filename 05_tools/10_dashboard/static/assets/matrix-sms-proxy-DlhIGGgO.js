async function e(e){e.innerHTML=`
    ...
    </div>`,Promise.all([t(),n(),r(),i()]).catch(e=>console.error(`SMS view error:`,e))}async function t(){let e=document.getElementById(`phonePresets`);if(e)try{let t=await(await fetch(`/api/matrix/accounts`)).json(),n=Array.isArray(t)?t:t.accounts||[],r=[];n.forEach(function(e){var t=e.phone||``;t&&t.length>=11&&r.indexOf(t)===-1&&r.push(t)}),e.innerHTML=r.length?r.map(function(e){var t=n.find(function(t){return t.phone===e});return`<div style="font-size:11px;padding:3px 4px;border-bottom:1px solid var(--border)">`+e+(t&&t.platform?` (`+t.platform+`)`:``)+`</div>`}).join(``):`<span style="font-size:11px;color:var(--text2)">无预设</span>`}catch(t){e.innerHTML=`<span style="color:var(--red);font-size:11px">❌ ${t.message}</span>`}}async function n(){let e=document.getElementById(`proxyList`);if(e)try{let t=(await(await fetch(`/api/matrix/proxies`)).json()).proxies||[];e.innerHTML=t.length?t.map(function(e){var t=e.proxy||`-`;return`<div style="font-size:11px;padding:3px 4px;border-bottom:1px solid var(--border)">`+(e.login_status===`logged_in`?`🟢`:`🔴`)+` `+e.account+` (`+e.platform+`) → `+t+`</div>`}).join(``):`<span style="font-size:11px;color:var(--text2)">无代理配置</span>`}catch(t){e.innerHTML=`<span style="color:var(--red);font-size:11px">❌ ${t.message}</span>`}}async function r(){let e=document.getElementById(`smsConfig`);if(e)try{let t=await(await fetch(`/api/matrix/sms/config`)).json();e.innerHTML=`
