import{i as e,r as t,t as n}from"./index-CpP93xH2.js";async function r(r){e(r,`读取联邦心跳中...`);try{let e=(await n(`/machines`)).machines||[],t=document.getElementById(`machineCount`);if(t&&(t.textContent=`共 ${e.length} 台主机`),!e.length){r.innerHTML=`<div class="error" style="padding:40px">无心跳数据</div>`;return}r.innerHTML=`
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:16px" id="machineGrid">
        ${e.map(e=>i(e)).join(``)}
      </div>
    `}catch(e){t(r,`加载失败: ${e.message}`)}}function i(e){let t=e._last_push_sec||999,n=e._live===!0||t<30,r=n||e.status===`online`&&t<120,i=e.status===`recent`||e.status===`online`&&t<300,a=n?`#1D9E75`:r?`#22C55E`:i?`#F59E0B`:`#EF4444`,o=e.minutes_ago||Math.round(t/60)||999,s=n?`${t}秒前`:o<60?`${o} 分钟前`:`${(o/60).toFixed(1)} 小时前`,c=e.disk_used_gb||0,l=e.disk_total_gb||1,u=Math.round(c/l*100),d=`
    ...
    <div style="background:var(--bg2);border-radius:var(--radius);padding:20px;border:1px solid ${r?`rgba(34,197,94,.2)`:`var(--border)`}">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
        <div style="display:flex;align-items:center;gap:10px">
          <span style="width:10px;height:10px;border-radius:50%;background:${a};display:inline-block;"></span>
          <div>
