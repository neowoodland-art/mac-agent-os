import{n as e,t}from"./index-CdKU4DyH.js";import{t as n}from"./account-selector-D87mja1n.js";var r=null,i=``,a=[],o={},s=localStorage.getItem(`_sc_api_url`)||`https://wx.tyhtak.com/api/biz/activity/api/v1/activity/recordswx1`;async function c(e,c){i=e.id||`sc`,o={},e.innerHTML=`
      ...
      <div style="background:var(--bg2);border-radius:10px;padding:14px;border:1px solid var(--border)">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-start">
          ...
          <div style="display:flex;flex-direction:column;gap:4px;flex:1;min-width:160px">
            ...
            <input id="interval_${i}" type="text" placeholder="间隔秒数 (如 60-180)" value="60-180"
    ...
    </div>`;try{let[e,o]=await Promise.all([t(`/matrix/accounts`),t(`/matrix/corpus`).catch(()=>({categories:[]}))]),s=Array.isArray(e)?e:e.accounts||[];a=o.categories||[];let c=document.getElementById(`acctList_${i}`);c&&(r=n(c,{accounts:s,checkAll:!1,height:`160px`,showBlueprint:!1,showCollectTime:!1}))}catch(e){let t=document.getElementById(`acctList_${i}`);t&&(t.innerHTML=`<div class="error">❌ ${e.message}</div>`)}let f=document.getElementById(`corpus_${i}`);f&&a.forEach(e=>{let t=document.createElement(`option`);t.value=e.name,t.textContent=`${e.name} (${(e.accessible||[]).join(`,`)||`通用`})`,f.appendChild(t)}),u(i),l(i)}function l(n){window[`_sc_ready_${n}`]||(window[`_sc_ready_${n}`]=!0,window._sc_testApi=async e=>{let n=document.getElementById(`apiUrl_${e}`)?.value,r=document.getElementById(`apiTestResult_${e}`);if(!n){r.textContent=`❌ 请输入 API 地址`;return}r.textContent=`⏳`;try{let e=await t(`/ops/import-topics`,{method:`POST`,body:JSON.stringify({api_url:n,page:1,page_size:1})});e.status===`ok`?(r.textContent=`✅ ${e.total} 条`,localStorage.setItem(`_sc_api_url`,n),s=n):r.textContent=`❌ ${e.message}`}catch(e){r.textContent=`❌ ${e.message}`}},window._sc_import=async e=>{let n=document.getElementById(`apiUrl_${e}`)?.value||s;if(!n){alert(`请先在⚙ 数据源设置中配置 API 地址`);return}localStorage.setItem(`_sc_api_url`,n),s=n;let r=document.getElementById(`importPanel_${e}`);r.style.display=`block`,r.innerHTML=`<div class="loading" style="padding:10px">⏳ 加载热帖列表...</div>`;try{let i=await t(`/ops/import-topics`,{method:`POST`,body:JSON.stringify({api_url:n,page:1,page_size:100})});if(i.status!==`ok`){r.innerHTML=`<div class="error">❌ ${i.message||`加载失败`}</div>`;return}let a=i.items||[];r.innerHTML=`
...
`),document.getElementById(`skipAnalysis_${e}`).checked=!0,document.getElementById(`importPanel_${e}`).style.display=`none`,document.getElementById(`result_${e}`).textContent=`✅ 已导入 ${n.length} 个视频（跳过分析模式）`},window._sc_analyze=async e=>{let n=r?.getSelected()||[],i=document.getElementById(`urls_${e}`)?.value,a=document.getElementById(`dir_${e}`)?.value||`praise`,o=document.getElementById(`corpus_${e}`)?.value||``,s=document.getElementById(`result_${e}`),c=document.getElementById(`preview_${e}`);if(!i||!i.trim()){s.textContent=`请填写至少一个视频链接`;return}let l=i.split(`
`).map(e=>e.trim()).filter(e=>e);if(!l.length){s.textContent=`请填写有效的视频链接`;return}if(!n.length){s.textContent=`请先选择要执行的账号`;return}s.textContent=`⏳ 正在分析 ${l.length} 个视频...`,c.style.display=`none`;let u=n[0].id;try{let r=(await t(`/ops/run`,{method:`POST`,body:JSON.stringify({type:`smart_comment`,accounts:[u],params:{urls:l,direction:a,corpus_category:o||void 0,preview:!0}})})).preview||{},i=Object.entries(r);if(!i.length){s.textContent=`⚠️ 分析完成但没有返回结果`;return}c.innerHTML=`
        ...
        <div style="clear:both"></div>`,c.style.display=`block`,s.textContent=`✅ 分析完成，${i.length} 个视频`}catch(e){s.textContent=`❌ 分析失败: ${e.message}`}},window._sc_confirmDistribute=async n=>{let i=r?.getSelected()||[];if(!i.length){alert(`请选择账号`);return}let a=document.querySelectorAll(`.sc-preview-input`),o={};if(a.forEach(e=>{o[e.getAttribute(`data-url`)]=e.value}),!Object.keys(o).length){alert(`没有可分发的评论`);return}let s=i.length*Object.keys(o).length;if(!await e(`确认分发到 ${i.length} 个账号`,`共 ${s} 条评论任务（P0 优先）`))return;let c=document.getElementById(`result_${n}`);c.textContent=`⏳ 正在分发 ${s} 条任务...`;let l=document.getElementById(`interval_${n}`)?.value||`60-180`;try{await t(`/ops/run`,{method:`POST`,body:JSON.stringify({type:`smart_comment`,accounts:i.map(e=>e.id),params:{urls:Object.keys(o),comments:o,direction:document.getElementById(`dir_${n}`)?.value||`praise`,interval:l}})}),c.textContent=`✅ 已分发 ${s} 条评论任务 (P0)`,setTimeout(()=>u(n),2e3)}catch(e){c.textContent=`❌ 分发失败: ${e.message}`}},window._sc_go=async n=>{let i=r?.getSelected()||[],a=document.getElementById(`urls_${n}`)?.value,s=document.getElementById(`dir_${n}`)?.value,c=document.getElementById(`corpus_${n}`)?.value||``,l=document.getElementById(`skipAnalysis_${n}`)?.checked,d=document.getElementById(`interval_${n}`)?.value||`60-180`,f=document.getElementById(`result_${n}`);if(!a||!a.trim()){f.textContent=`请填写至少一个视频链接`;return}let p=a.split(`
`).map(e=>e.trim()).filter(e=>e);if(!i.length){f.textContent=`请先选择账号`;return}let m=i.length*p.length,h=`账号 ${i.length} 个 · 视频 ${p.length} 个\n方向: ${s} · 间隔: ${d}${l?` · ⏭ 跳过分析`:``}`;if(!await e(`即将${l?``:`分析并`}分发 ${m} 条评论`,h)){f.textContent=`已取消`;return}f.textContent=`⏳ 分发 ${m} 条任务...`;try{let e=await t(`/ops/run`,{method:`POST`,body:JSON.stringify({type:`smart_comment`,accounts:i.map(e=>e.id),params:{urls:p,direction:s,corpus_category:c||void 0,interval:d,preview:!1,skip_analysis:l&&Object.keys(o).length>0,titles:l?o:{},video_ids:p.map(e=>``)}})}),r=e?.per_machine||{},a=``,h=Object.keys(r).length>0;for(let[e,t]of Object.entries(r))a+=`🖥 ${e}: ✅${t?.success||0} ❌${t?.failed||0}\n`;let g=e?.commands?.length||0;f.textContent=h?a:g>0?`✅ 已提交 ${g} 条任务，正在排队执行`:`✅ 已分发 ${m} 条评论任务`,setTimeout(()=>u(n),2e3)}catch(e){f.textContent=`❌ 执行失败: ${e.message}`}})}async function u(e){let n=document.getElementById(`batchLogs_${e}`);if(n)try{let e=(await t(`/ops/batch-logs?limit=30`)).logs||[];if(!e.length){n.innerHTML=``;return}n.innerHTML=`
      <details open>
        ...
        <div style="font-size:10px">
          ${e.map((e,t)=>{let n=e.machine_counts||{},r=Object.entries(n).map(([e,t])=>`${e}:✅${t.success||0}❌${t.failed||0}`).join(` `);return`<div style="background:var(--bg2);border:1px solid var(--border);border-radius:4px;padding:6px 8px;margin-bottom:3px;cursor:pointer" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'block':'none'">
            ...
            <div style="display:none;padding:4px 8px;margin-bottom:4px;font-size:9px;color:var(--text2);background:var(--bg3);border-radius:0 0 4px 4px">
              <div>方向: ${e.direction||`-`} · 间隔: ${e.interval||`-`}</div>
