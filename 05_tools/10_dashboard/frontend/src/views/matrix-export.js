export async function loadView(container) {
  ...
  try {
    const [r1, r2] = await Promise.all([
      fetch('/api/matrix/export/list').then(r => r.ok ? r.json() : { exports: [] }).catch(() => ({ exports: [] })),
      fetch('/api/matrix/profiles').then(r => r.ok ? r.json() : { profiles: {} }).catch(() => ({ profiles: {} })),
    ]);
    const list = r1.exports || [];
    container.innerHTML = `
      <div style="font-size:18px;font-weight:600;margin-bottom:12px">📦 导入导出</div>
      ${list.length ? `<div style="font-size:12px;margin-bottom:8px">已有导出包:</div>
        <table style="width:100%;font-size:11px;border-collapse:collapse">
          ...
          ${list.map(e => `<tr style="border-bottom:1px solid var(--border)"><td style="padding:4px">${e.name}</td><td style="padding:4px">${e.count||0}</td><td style="padding:4px">${e.created_at||'-'}</td><td style="padding:4px"><button onclick="fetch('/api/matrix/export/download/${e.id}').then(r=>r.blob()).then(b=>{const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='${e.name}.json';a.click()})" style="background:var(--bg3);border:1px solid var(--border);padding:2px 8px;border-radius:4px;cursor:pointer;font-size:10px">⬇下载</button></td></tr>`).join('')}
