...
export async function loadView(container) {
  ...
  try {
        ...
        groups[m].forEach(a => {
          ...
          html += `<tr style="border-bottom:1px solid var(--border)" data-filter="${(a.id+a.phone+a.nickname+a.identity_dir+m).toLowerCase()}">`
            ...
            + `<td style="padding:3px 6px;white-space:nowrap">
              <button onclick="window._actCol(this,'${a.id.replace(/'/g,"\\'")}','${a.platform}')" style="background:transparent;border:1px solid var(--border);padding:1px 5px;border-radius:4px;cursor:pointer;font-size:10px;color:var(--text)" title="采集">📡</button>
              <button onclick="window._actLogin(this,'${a.id.replace(/'/g,"\\'")}','${a.platform}')" style="background:transparent;border:1px solid var(--border);padding:1px 5px;border-radius:4px;cursor:pointer;font-size:10px;color:var(--text)" title="登录">🔑</button>
            </td></tr>`;
        });
      });
      html += '</table>';
    ...
    // ── 单账号操作按钮 ──
    window._actCol = async (btn, aid, platform) => {
    ...
    window._actLogin = async (btn, aid) => {
      if (btn) { btn.textContent = '⏳'; btn.disabled = true; }
      try {
    ...
    window.showCreateAccount = window._showCreateAcct = () => {
      // 可选机器列表（不重复+本机排第一）
      const machines = [...new Set(allAccounts.map(a => a.owner_machine).filter(Boolean))];
      ...
      // 检查是否重复
      const checkDup = (plat, phone) => allAccounts.find(a => a.platform === plat && a.phone === phone && a.enabled !== false);
      ...
      overlay.innerHTML = `
        <div style="background:var(--bg2);border-radius:12px;padding:20px;max-width:380px;width:90%">
          ...
          <div style="display:grid;gap:6px;font-size:12px">
            ...
            <label>手机号 <input id="newAcctPhone" oninput="_updateAcctId()" placeholder="手机号" autocomplete="off" style="width:100%;padding:4px;background:var(--bg3);border:1px solid var(--border);color:var(--text);border-radius:4px"></label>
