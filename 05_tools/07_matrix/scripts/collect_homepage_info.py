...
async def extract_xiaohongshu(page, identity_name, phone):
    ...
    try:
            ...
            try:
                if method == 0:
                    await page.evaluate("""()=>{
                        ...
                        for(const e of els){
                            if(e.textContent.trim()==='我'&&e.offsetParent!==null){
                                e.click();return true;
                            }
                        }
                        return false;
                    ...
                    # 找 profile 链接
                    await page.evaluate("""()=>{
                        const a=document.querySelector('a[href*="user/profile"], a[href*="profile"], a[href*="user"]');
        ...
        all_text2 = "\n".join(dom_data2.get("texts", []))
        lines2 = [l.strip() for l in all_text2.split("\n") if l.strip()]
        text_for_nums = "\n".join(lines2[:400])
        ...
        if not info["fans"] and not info["nickname"]:
            print(f"   ⚠️ 可能未登录 (lines={len(lines2)})")
        return info
    except Exception as e:
        print(f"   ❌ 小红书异常: {e}")
        info["status"]="error"; info["_error"]=str(e)
