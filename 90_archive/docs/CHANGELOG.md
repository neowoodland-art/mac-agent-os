...
- 新增 `REVIEW-V2.0.md`，正式记录原形、当前实现与后续升级设想之间的差异
- 新增 `V2.0-SUMMARY.md`，作为面向使用者的最新说明汇总
- 重写 `CORE-ARCHITECTURE.md`，将当前仓库冻结为 AgentOS v2.0 基准定义
- 重写 `README.md`，统一项目入口与当前基准说明
- 更新 `QUICKSTART.md`，补充双根目录与软链接边界
- 重写 `SKILLS-CATALOG.md`，收口为 v2.0 技能目录
- 更新 `REQUIREMENTS.md`，移除过期运行状态并对齐 v2.0 口径
- 将 `AgentOS-完整说明文档_副本.md` 正式归档到 `90_archive/docs/`，原路径保留跳转页

### 架构决策

- 明确 `agent-os/` 与 `agent-os-local/` 的双根目录边界
- 明确 `04_memory/` 中共享内容与本机私有内容的分界
- 明确 v2.0 采用“双入口汇聚到 Inbox”的知识流定义
- 明确 `05_tools/` 是工具层，不默认等于技能层
- 明确 `category` / `SKILL_CARD.yaml` / `collect_from_local.py` 等能力延后到后续版本
